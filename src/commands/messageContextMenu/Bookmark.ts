...
export async function bookmarkMessageContextMenu(interaction: APIMessageApplicationCommandInteraction) {
  ...
  const tagsToAdd = [];
  tagsToAdd.push(await upsertTag(user.id, `channel:${resolvedMessage.channel_id}`, true));
  tagsToAdd.push(await upsertTag(user.id, `author:${resolvedMessage.author.id}`, true));
  if (interaction.guild_id) {
    tagsToAdd.push(await upsertTag(user.id, `guild:${interaction.guild_id}`, true));
  }
